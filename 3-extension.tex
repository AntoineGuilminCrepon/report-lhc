...
In both cases, we simply "unfold" the terms using the corresponding $\mathbf{wp}_{\forall}$ and $\mathbf{wp}_{\exists}$ rules, and then a number \textsc{lockstep-swap} to complete the proof.
...
\textsc{lockstep-while} can be derived from \textsc{lockstep-star} and \textsc{lockstep-assume} this way:
...
Both leaves can be derived from the premisse of \textsc{lockstep-while} by applying \textsc{assume} (after negating the guards for the righthand leave), and collapsing the postcondition according to the return value being forced by the rule.
...
\begin{scprooftree}{0.75}
    ...
    {\fun b'. \WPE {\m[j: \code{assume}(g_j') | j \in J]}*
    {\begin{pmatrix*}[l]
    \fun b. (b \cdot b' =_{I\cup J} 0 \land Q) \\
    \lor 
    \begin{pmatrix*}[l]
...
\begin{scprooftree}{0.75}
    ...
    {\fun b'. \WPE {\m[j: \lnot g_j' | j \in J]}*
    {\begin{pmatrix*}[l]
    \fun b. (b \cdot b' \ne_{I\cup J} 0 \land Q) \\
    \lor 
    \begin{pmatrix*}[l]
        b \cdot b' =_{I\cup J} 0 \\
        \land\WPU {\m[i: t_i | i \in I]}{\WPE {\m[j: t_j' | j \in J]} {P}}
    \end{pmatrix*}
    \end{pmatrix*}}}$}
    \UnaryInfC{$\V
    ...
    {\fun b'. \WPE {\m[j: \code{assume}(\lnot g_j') | j \in J]}*
    {\begin{pmatrix*}[l]
    \fun b. (b \cdot b' \ne_{I\cup J} 0 \land Q) \\
    \lor 
    \begin{pmatrix*}[l]
