...
\textsc{wp-while} can be derived from \textsc{wp-star} and \textsc{wp-assume} this way:
...
Both leaves can be derived from the premisse of \textsc{wp-while} by applying \textsc{assume} (after negating the guards for the righthand leave), and collapsing the postcondition according to the return value being forced by the rule.
