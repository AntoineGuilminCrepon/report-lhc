...
\Cref{rule:wpE-exists} mirrors \cref{rule:wpU-all}, but uses existential quantifiers instead of universal ones. Without much surprise, $\mathbf{wp}_{\exists}$ works better with existentials. \Cref{rule:wpE-while} was modified to take into account the necessity of projectability for $\mathbf{wp}_{\exists}$ statements.
...
\begin{mathfig}{\small}
    \begin{proofrules}
        \input{rules/lockstep-refine.tex}
        \label{rule:lockstep-refine}

        \input{rules/lockstep-proj.tex}
...
\begin{proof}
    ...
    The left-side assumption is proved the following way: given that both terms receive the same input, if we chose the same trace to execute in both terms, we then get the same output (\ie every term is "deterministic" in the context of $\mathbf{wp}_{\exists}$).
...
In both cases, we simply "unfold" the terms using the corresponding $\mathbf{wp}_{\forall}$ and $\mathbf{wp}_{\exists}$ rules, and then a number \cref{rule:lockstep-swap} to complete the proof.
